...
def pause_requested() -> bool:
    ...
    """
    path = os.environ.get("EXEX_PAUSE_REQUEST")
...
def mark_paused() -> None:
    ...
    exits nor requeues; Exex must still observe success and retain the checkpoint.
    """
    Path(os.environ["EXEX_PAUSE_READY"]).touch()


def link(name: str, url: str) -> None:
    """Record a named URL for this task; do nothing outside an Exex job.
    ...
    """
    destination = os.environ.get("EXEX_LINKS_FILE")
    if not destination:
        return
