...
from exex import xm
from exex.xm import executables
from exex.xm import job_blocks
from exex.xm import utils
...
class PythonPackage(job_blocks.ExecutableSpec):
    ...
    Note:
        exex uses ``pip install`` to install the package into a temporary
        ...
        exex uses ``pip install --no-deps`` by default to install the package.
        This means that exex **will not** install the dependencies required by
...
class UniversalPackage(job_blocks.ExecutableSpec):
    ...
    as it can be used for any language and build system not supported natived by Exex.
...
class PexBinary(job_blocks.ExecutableSpec):
    ...
    Notes:
        A few assumptions is currently hardcoded in Exex:
    ...
    Args:
        ...
        dependencies: List of dependencies to include in the PEX. These are put inside
            the _archive_ created by Exex as opposed to being bundled inside by the PEX
...
class ShifterContainer(_ContainerSpec):
    ...
    passed through unchanged; Exex does not build, import or resolve images.
