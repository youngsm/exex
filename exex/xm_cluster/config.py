...
EXEX_CONFIG = flags.DEFINE_string(
    "exex_config", os.environ.get("EXEX_CONFIG"), "Path to exex configuration."
)
...
class LocalSettings:
    ...
    def storage_root(self) -> str:
        if "storage" not in self._data:
            self._data["storage"] = {"staging": os.path.join(os.getcwd(), ".exex")}
...
class ClusterSettings:
    ...
    def storage_root(self):
        if "storage" not in self._data:
            self._data["storage"] = {"staging": "exex-staging"}
        return self._data["storage"]["staging"]
...
class Config:
    ...
    def _snapshot(self) -> "Config":
        ...
        snapshot._environment = {
            key: self._environment.get(key) for key in ("EXEX_PROJECT", "EXEX_CLUSTER")
        }
    ...
    def project(self) -> Optional[str]:
        project = self._environment.get("EXEX_PROJECT")
    ...
    def default_cluster(self) -> str:
        if "clusters" not in self._data:
            self._data["clusters"] = []
        cluster = self._environment.get("EXEX_CLUSTER")
        if cluster is None:
            if not self._data.get("clusters", None):
                raise ValueError(
                    ...
                    "Refer to\n\n"
                    "  https://github.com/youngsm/exex/blob/main/docs/configuration.md\n\n"
    ...
    def default(cls):
        return cls(
            {
                "project": None,
                "local": {"storage": {"staging": os.path.join(os.getcwd(), ".exex")}},
                "clusters": [],
            }
        )
...
def default() -> Config:
    ...
    try:
        if EXEX_CONFIG.value is not None:
            logging.debug("Loading config from %s", EXEX_CONFIG.value)
            return Config.from_file(EXEX_CONFIG.value)
    except flags.UnparsedFlagAccessError:
        logging.debug("Unable to load config from flag")
    # Use file from environment variable
    exex_config_path = os.environ.get("EXEX_CONFIG", None)
    if exex_config_path is not None:
        logging.debug("Loading config from %s", exex_config_path)
        return Config.from_file(exex_config_path)
    cwd_path = os.path.join(os.getcwd(), "exex.toml")
    if os.path.exists(cwd_path):
        logging.debug("Loading config from %s", cwd_path)
        return Config.from_file(cwd_path)
    user_config_path = os.path.join(appdirs.user_config_dir("exex"), "config.toml")
