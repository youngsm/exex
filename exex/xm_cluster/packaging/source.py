...
from exex.xm_cluster import executable_specs as specs
from exex.xm_cluster.packaging.create_archive import _create_entrypoint_cmds
...
def _selected_files(source: specs.SourceTree):
    ...
    for name in sorted({Path(name) for name in names}):
        ...
        if name.parts[0] in {"job-param.sh", ".environment"}:
            raise ValueError(f"Source path is reserved for Exex runtime files: {name}")
...
def freeze(source: specs.SourceTree, local_storage_root: str) -> specs.FrozenSource:
    ...
    entrypoint = "bash -e -c " + shlex.quote(_create_entrypoint_cmds(source)) + " exex"
    with tempfile.TemporaryDirectory(prefix="exex-source-") as temporary:
